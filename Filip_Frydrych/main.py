...
RUN_TIME=9                             #main program part working time in minutes (max 9 minutes)
...
accelerator_raw = sense.get_accelerometer_raw() #getting accelerator values
accelerator_x.append(accelerator_raw["x"])     #appending accelerator x value to the gyro x table
accelerator_y.append(accelerator_raw["y"])      #appending accelerator y value to the gyro y table
accelerator_z.append(accelerator_raw["z"])      #appending accelerator z value to the gyro z table
...
while (now_time <= (start_time + timedelta(minutes=RUN_TIME))):            #run for RUN_TIME in minutes
    ...
    if now_time >= (loop_time + timedelta(seconds=ITERATION_TIME)):        #after every ITERATION_TIME seconds do calculations
        ...
        point = iss1.coordinates()
        total_R=6370+point.elevation.km          #Current altitude of ISS, meters above the ground
        ...
        accelerator_y.append(accelerator_raw["y"])      #appending accelerator y value to the gyro y table
        accelerator_z.append(accelerator_raw["z"])      #appending accelerator z value to the gyro z table
        print("ax= "+str(accelerator_raw["x"])+" ay= "+str(accelerator_raw["y"])+" az= "+str(accelerator_raw["z"]))  #printing actually degrees per sec. speed
...
L=[] 
for j in range (0,i):
    L.append("time_f=" + str(time_f[j]) + ", lat=" + str(lat[j]) + ", lon=" + str(lon[j]) + ", alt=" + str(alt[j]) + ", v1=" + str(v1[j]) + ", v2=" + str(v2[j]) + ", gx=" + str(gyro_x[j]) + ", gy=" + str(gyro_y[j]) + ", gz=" + str(gyro_z[j]) + ", accelx=" + str(accelerator_x[j]) + ", acely=" + str(accelerator_y[j]) +", accelz=" + str(accelerator_z[j]) + "\n")
