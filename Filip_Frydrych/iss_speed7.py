...
#data acquired, final calculations and results saving
    ...
    gmax=max(gxm,gym,gzm)
    
    print("gx=" + str(gxm) + "gy=" + str(gym) + "gz=" + str(gzm))
    
    ISS_T = (360/gmax)/60
    print("ISS time for full orbit:" +str(ISS_T) + " min")

    estimate_kmps=2*3.14159*(6371.0 + numpy.median(alt))/(ISS_T*60)
    estimate_kmps_formatted2 = "{:.4f}".format(estimate_kmps)
    # Create a string to write to the file
    output_string = estimate_kmps_formatted2
    print("ISS speed calculated from gyroscope:" + output_string + "km/s" )
    # Write to the file
    ...
    L=[]
    for j in range (0,i):
        L.append("time_f=" + str(time_f[j]) + ", lat=" + str(lat[j]) + ", lon=" + str(lon[j]) + ", alt=" + str(alt[j]) + ", v1=" + str(v1[j]) + ", v2=" + str(v2[j]) + ", gx=" + str(gyro_x[j]) + ", gy=" + str(gyro_y[j]) + ", gz=" + str(gyro_z[j]) + "\n")
